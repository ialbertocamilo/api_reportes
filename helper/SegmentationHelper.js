...
exports.loadUsersSegmented = async (course_id,only_active_users=true) => {
  ...
  for (segment of segments_groupby) {
    ...
    const [rows] =
      await con.raw(`select users.id ,users.subworkspace_id ,users.name,users.lastname,users.surname,users.email, users.phone_number, users.document ,sc.grade_average,sc.advanced_percentage,sc.status_id,sc.created_at as sc_created_at,
