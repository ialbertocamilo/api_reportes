...
async function notasUsuario2 ({ document, course_ids=[] }) {
      ...
      if (compatiblesCoursesIds.length) {
        ...
        if (compatiblesUserSummaryCourses.length) {
          ...
          if (highestGrade > 0) {
            ...
            compatible.course_name = _course ? _course.name : '-'
            userSummaryCourses.push(compatible)
          }
        }
      }else{
        const _course =  await getCourse(courseId);
        const _status = userCourseStatuses.find( uc => uc.code =='desarrollo')
        userSummaryCourses.push({
          user_id:user.id,
          course_id:courseId,
          course_name : _course ? _course.name : '-',
          grade_average: '-',
          views:'-',
          advanced_percentage:'-',
          restarts:'-',
          qualification_type_id:_course.qualification_type_id,
    ...
    if(userSummaryTopics.length > 0){
      ...
      const [topics] = await con.raw(`
       select t.name as topic_name, 
              ...
              t.qualification_type_id
       from topics as t 
       where t.course_id = :courseId and t.active = 1 and t.deleted_at is null 
       `,{courseId:course_id});
        for (const topic of topics){
          const qualification = QualificationTypes[topic.qualification_type_id];
          topicsArray.push({
            tema:topic.topic_name,
            tipo_calificacion:qualification ? qualification.name : '-',
            nota:'-',
            puntaje:'-',
            correctas:'-',
            incorrectas:'-',
